   ...
   public static class ResourceManager
    {
       ...
       /* Author: Alex DS */
        public static int LoadAudio(string audioName)
        {
            int audio;
            if (!audioDictionary.TryGetValue(audioName, out audio))
            {
                ...
                byte[] sound_data = LoadWave(
                    File.Open(audioName, FileMode.Open),
                    out channels,
                    out bits_per_sample,
                    out sample_rate);
                ...
                if (AL.GetError() != ALError.NoError)
                {
                    ...
                    throw new Exception("Error occured while trying to read in AudioFile:"+audioName);
                }
                audioDictionary.Add(audioName, audio);
            }
            return audio;
        }
